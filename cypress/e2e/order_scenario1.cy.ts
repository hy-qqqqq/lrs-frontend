// cypress/e2e/order_scenario1.cy.ts

describe('Order Management Scenario 1', () => {
    ...
    it('User1 should reject the created order', () => {
        cy.fixture('user').then((userData) => {
            const user1 = userData.users[1];
            cy.visit('/login');
        ...
        cy.get('.v-card').last().contains('button', 'Reject').click();
        cy.wait(2000); // 等待請求完成
    ...
    it('User0 should delete the rejected order', () => {
        cy.fixture('user').then((userData) => {
            const user0 = userData.users[0];
            cy.visit('/login');
            cy.get('input[name="username"]').type(user0.username);
